...
OpenAI Codex CLI의 기존 `codex resume` 화면을 그대로 사용하면서, 대화 세션을 프로젝트 폴더별로 묶어 보여주는 비공식 소스 패치 및 업데이트 관리자입니다.
...
- `codex resume` 첫 화면에서 세션을 작업 디렉터리(`cwd`)별 프로젝트로 묶습니다.
...
- 공식 `codex update`가 끝난 뒤 같은 패치를 새 버전 소스에 적용하고 CLI를 다시 빌드합니다.
...
| 프로젝트 관리 데이터베이스 | 아닙니다. 기존 세션에 기록된 작업 디렉터리를 프로젝트 기준으로 사용합니다. |
...
`Enter`를 누르면 `~/Isaac`을 작업 디렉터리로 사용하는 새 Codex 세션이 시작됩니다. 해당 폴더의 `.codex` 설정과 프로젝트 범위 설정도 새 작업 디렉터리를 기준으로 다시 로드됩니다.
...
이 도구에서 프로젝트는 **세션에 저장된 작업 디렉터리 경로**입니다.

- 같은 `cwd`를 가진 세션은 하나의 프로젝트로 묶입니다.
- 프로젝트 이름은 기본적으로 경로의 마지막 폴더명입니다.
- 작업 디렉터리 정보가 없는 세션은 프로젝트로 묶지 않고 개별 세션으로 남을 수 있습니다.
- 폴더 이름을 변경하거나 이동하면 과거 경로와 새 경로가 서로 다른 프로젝트로 보일 수 있습니다.
- Codex Desktop에서 프로젝트를 만들기만 하고 그 폴더에서 세션을 한 번도 만들지 않았다면 CLI 목록에 표시할 세션 정보가 없을 수 있습니다.

즉, Desktop의 프로젝트 정의를 직접 가져오는 기능이 아니라 **이미 존재하는 Codex 세션을 경로 기준으로 정리하는 기능**입니다.
...
2. 새 npm 패키지 버전 확인
3. 같은 버전의 공식 Rust 태그 가져오기
4. 패치 적용 가능 여부 검사
5. 패치된 CLI 릴리스 빌드
6. 공식 런타임 구성 요소 복구
7. `codex` 런처를 커스텀 관리자로 다시 연결

예약 작업에서도 반드시 `codex update`를 호출하면 같은 흐름이 적용됩니다. npm 명령으로 Codex만 직접 교체하면 커스텀 재빌드 단계가 실행되지 않을 수 있습니다.

현재 패치는 Codex CLI `0.150.1`에서 검증했습니다. 이후 버전에서도 소스 구조가 호환되면 자동 적용되지만, 업스트림 리팩터링으로 충돌할 경우 수동 패치 갱신이 필요합니다.
...
구성 요소가 빠졌거나 공식 버전과 커스텀 버전이 다르면 재조립 또는 재빌드를 시도합니다. 패치 적용이나 빌드가 실패하면 경고를 출력하고 공식 `codex-upstream`을 실행합니다.
...
- 기존 Codex가 사용하는 세션 목록을 화면에서 경로별로 재구성할 뿐입니다.
...
- 프로젝트 그룹화, 프로젝트 진입/복귀, 새 세션 경로 전달 테스트
- 네이티브 TUI 스냅샷 테스트
- 기존 resume, archive, remote, fork 동작 회귀 테스트
- 관련 `resume_picker` 테스트 119개 통과
- 깨끗한 Codex `rust-v0.150.1` 태그에 패치 적용 확인
